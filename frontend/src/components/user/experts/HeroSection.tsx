...
const HeroSection: React.FC<ResultsInfoProps> = ({ totalCount }) => {
 const roundedCount = Math.floor(totalCount / 5) * 5;
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center">
          <div className="inline-flex items-center bg-[#015F4A]/5 border border-[#015F4A]/20 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-[#015F4A] font-medium text-sm">
              Licensed Mental Health Professionals
            </span>
          </div>

          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 leading-tight">
            Expert <span className="text-[#015F4A]">Counsellors</span>
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-xl mx-auto">
            {stats.map((stat, index) => (
              <div
                key={index}
                className="bg-white border border-[#015F4A]/10 rounded-lg p-4 text-center"
              >
