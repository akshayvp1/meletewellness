...
// import adminAuthService from '@/services/admin/CounsellorManagingService';
...
const AddCounsellor: React.FC = () => {
  ...
  const languageOptions: string[] = ["English", "Malayalam", "Tamil", "Hindi"];
  ...
  const validateForm = (): Errors => {
    ...
    if (formData.email.trim()) {
      if (!formData.phone.trim()) {
        ...
        newErrors.phone =
          "Phone number must be 10 digits, start with 6, 7, 8, or 9, and contain only numbers.";
      }
    ...
    if (formData.specialization.trim().length > 200) {
      newErrors.specialization =
        "Specialization must be 200 characters or less.";
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await CounsellorManagingService.addCounsellor(counsellorData);
      ...
      if (error.message !== "Please correct the form errors.") {
        setErrors({
          general:
            error.message || "Error adding counsellor. Please try again.",
        });
      }
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const fetchedEvents = await CounsellorManagingService.getCounsellors();
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Fill in the details to add a new counsellor to the platform
        </p>
      </div>
