...
export const checkUserActiveStatus = async (
  ...
  try {
    ...
    if (user.role !== "user") {
      res.status(403).json({ success: false, message: "Access forbidden: Users only" });
      return;
    }

    // Placeholder: Implement userAuthService.checkActiveStatus
    // const isActive = await userAuthService.checkActiveStatus(user.id);
    const isActive = true; // Replace with actual service call
